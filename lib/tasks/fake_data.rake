    ...
    for i in 0...10
      ...
      for a in 0...3
        ...
        for vg in 0...arm.visit_count
          VisitGroup.create(sparc_id: sparc_id, arm_id: arm.id, position: vg+1, name: rand_string, day: today, window_before: rand, window_after: rand)
        end
        for li in 0...rand
          #for the service_id, it simply uses the counting index so each lineitem created there won't be duplicate services
          LineItem.create(sparc_id: sparc_id, arm_id: arm.id, service_id: services[li].id, sparc_core_name: rand_core)
        end
        arm.visit_groups.each do |vg|
          arm.line_items.each do |li|
            Visit.create(sparc_id: sparc_id, line_item_id: li.id, visit_group_id: vg.id, research_billing_qty: rand, insurance_billing_qty: rand, effort_billing_qty: rand)
          end
        end
      end
    end
