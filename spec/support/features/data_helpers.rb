module Features
  module DataHelpers
    ...
    def create_populated_protocol_data
      ...
      @line_item = create(:line_item, arm_id: @arm.id, service_id: @service.id, protocol_id: @protocol.id)
      @visit = create(:visit, line_item_id: @line_item.id, visit_group_id: @visit_group.id)
      @user = create(:identity)
      @project_role = create(:project_role, identity: @user, protocol: @protocol, role: 'primary-pi')
    end
  end
end
