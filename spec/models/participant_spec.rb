...
RSpec.describe Participant, type: :model do
  ...
  context 'class methods' do
    ...
    let!(:visit_group1) { create(:visit_group, arm: arm, name: 'Turd', position: 1, day: 1) }
    let!(:visit_group2) { create(:visit_group, arm: arm, name: 'Ferguson', position: 2, day: 2) }
    ...
    describe 'build appointments' do
      ...
      it 'should create additional appointments if a visit group is added to the arm' do
        participant.build_appointments
        create(:visit_group, arm: participant.arm, name: "Dirk", position: 3, day: 3)
        arm.reload
        participant.build_appointments
