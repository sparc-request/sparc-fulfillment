...
feature 'Report form validations', js: true do
  ...
  context 'Identity submits Invoice Report request form with missing date' do
    scenario 'and sees a blank start date error' do
      given_i_have_opened_the_blank_invoice_report_form_modal
      when_i_click_request_report
      then_i_should_see_an_error_saying "Start date must not be blank"
    end
  end

  context 'Identity submits Invoice Report request form with a missing title' do
  ...
  def given_i_have_opened_the_blank_invoice_report_form_modal
    protocol = create_and_assign_protocol_to_me
    create(:participant, protocol: protocol)
    visit documents_path

    find("button[data-type='invoice_report']").click
    wait_for_ajax
  end
