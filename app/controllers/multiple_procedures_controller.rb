class MultipleProceduresController < ApplicationController
  ...
  def incomplete_all
    @core_id = params[:core_id]
    procedures = Procedure.where(sparc_core_id: params[:core_id], appointment_id: params[:appointment_id])

    #Create test note for validation
    @note = Note.new(kind: 'reason', reason: params[:reason])

    if @note.valid?
      #now update all procedures with new status and create notes.
      procedures.each do |procedure|
        procedure.update_attributes(status: params[:status], performer_id: current_identity.id)
        procedure.notes.create(identity_id: currenty_identity.id, kind: 'reason', reason: params[:reason], comment: params[:comment])
      end
    end
  end
